/* eslint-env browser */
const APIBASE = 'https://fling.prayer.cf/api'
...
function getWeather() {
  if (userSettings().location) {
    superagent.get('https://fling.prayer.cf/api/weather/' + userSettings().location).end((err, res) => {
      if(!err)
...
function closesettings() {
  ...
  if (localStorage.auth){
    superagent.post(`${APIBASE}/settings`)
     .send(settings)
     .set('Authorization', localStorage.auth)
     .end()
  }
...
function rotationNation() {
  superagent.get(`${APIBASE}/background`)
  .set('Authorization', localStorage.auth)
  .end((err,res) => {
...
function getSettings() {
  ...
  superagent.get(`${APIBASE}/settings`)
  .set('Authorization', localStorage.auth)
  .end((err, res) => {
...
function removeTodo(id) {
  superagent.del(`${APIBASE}/todo/${id}`)
  .set('Authorization', localStorage.auth)
  .end((err, res) => {
    todos()
  })
...
document.addEventListener('DOMContentLoaded', function () {
  ...
  document.getElementsByClassName('settingsbtn')[0].addEventListener('click', opensettings);
  document.getElementsByClassName('settingsbtn')[1].addEventListener('click', closesettings);
