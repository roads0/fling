...
!(function(e) {
  ...
  let t = {
    ...
    let a, c, d, f, g, h, i, l, o, p, r, s, u; for (e = e.replace(/^ +$/gm, ""); e;) {
      if ((i = this.rules.newline.exec(e)) && (e = e.substring(i[0].length), i[0].length > 1 && this.tokens.push({type: "space"})), i = this.rules.code.exec(e)) {
        ...
        e = e.substring(i[0].length), this.tokens.push({type: "blockquote_start"}), i = i[0].replace(/^ *> ?/gm, ""), this.token(i, n), this.tokens.push({type: "blockquote_end"})
        ...
        e = e.substring(i[0].length), i[3] && (i[3] = i[3].substring(1, i[3].length - 1)), u = i[1].toLowerCase().replace(/\s+/g, " "), this.tokens.links[u] || (this.tokens.links[u] = {
          href: i[2],
          title: i[3]
        })
    ...
    if (this.options = t || m.defaults, this.links = e, this.rules = r.normal, this.renderer = this.options.renderer || new i(), this.renderer.options = this.options, !this.links) {
    ...
    for (var i, l, n, o = "", r, t; e;) {
      if (l = this.rules.escape.exec(e)) {
        e = e.substring(l[0].length), o += l[1]
      } else if (l = this.rules.autolink.exec(e)) {
        e = e.substring(l[0].length), r = l[2] === "@" ? `mailto:${n = a(this.mangle(l[1]))}` : n = a(l[1]), o += this.renderer.link(r, null, n)
      } else if (this.inLink || !(l = this.rules.url.exec(e))) {
        if (l = this.rules.tag.exec(e)) {
          !this.inLink && (/^<a /i).test(l[0]) ? this.inLink = !0 : this.inLink && (/^<\/a>/i).test(l[0]) && (this.inLink = !1), e = e.substring(l[0].length), o += this.options.sanitize ? this.options.sanitizer ? this.options.sanitizer(l[0]) : a(l[0]) : l[0]
        } else if (l = this.rules.link.exec(e)) {
          e = e.substring(l[0].length), this.inLink = !0, r = l[2], this.options.pedantic ? (t = (/^([^'"]*[^\s])\s+(['"])(.*)\2/).exec(r)) ? (r = t[1], i = t[3]) : i = "" : i = l[3] ? l[3].slice(1, -1) : "", r = r.trim().replace(/^<([\s\S]*)>$/, "$1"), o += this.outputLink(l, {
            href: s.escapes(r),
            title: s.escapes(i)
          }), this.inLink = !1
          ...
          if (e = e.substring(l[0].length), t = (l[2] || l[1]).replace(/\s+/g, " "), !(t = this.links[t.toLowerCase()]) || !t.href) {
            o += l[0].charAt(0), e = l[0].substring(1) + e; continue
          } this.inLink = !0, o += this.outputLink(l, t), this.inLink = !1
        } else if (l = this.rules.strong.exec(e)) {
          e = e.substring(l[0].length), o += this.renderer.strong(this.output(l[4] || l[3] || l[2] || l[1]))
        } else if (l = this.rules.em.exec(e)) {
          e = e.substring(l[0].length), o += this.renderer.em(this.output(l[6] || l[5] || l[4] || l[3] || l[2] || l[1]))
        ...
        l[0] = this.rules._backpedal.exec(l[0])[0], e = e.substring(l[0].length), l[2] === "@" ? r = `mailto:${n = a(l[0])}` : (n = a(l[0]), r = l[1] === "www." ? `http://${n}` : n), o += this.renderer.link(r, null, n)
      }
    }
    ...
    if (this.options.highlight) {
      let r = this.options.highlight(e, t); r != null && r !== e && (n = !0, e = r)
    }
    ...
    } this.options.baseUrl && !g.test(e) && (e = u(this.options.baseUrl, e)); try {
    ...
    this.options.baseUrl && !g.test(e) && (e = u(this.options.baseUrl, e)); let r = `<img src="${e}" alt="${n}"`
    ...
    this.inline = new s(e.links, this.options), this.inlineText = new s(e.links, f({}, this.options, {renderer: new l()})), this.tokens = e.reverse(); for (var t = ""; this.next();) {
      t += this.tok()
    }
    ...
    case "space": return ""; case "hr": return this.renderer.hr(); case "heading": return this.renderer.heading(this.inline.output(this.token.text), this.token.depth, h(this.inlineText.output(this.token.text))); case "code": return this.renderer.code(this.token.text, this.token.lang, this.token.escaped); case "table": var e,
      i = "",
      n, r,
      s = "",
      t; for (n = "", e = 0; e < this.token.header.length; e++) {
      ...
      return this.renderer.table(s, i); case "blockquote_start": for (i = ""; this.next().type !== "blockquote_end";) {
      i += this.tok()
    }

      return this.renderer.blockquote(i); case "list_start": i = ""; for (var l = this.token.ordered, o = this.token.start; this.next().type !== "list_end";) {
      i += this.tok()
    }

      return this.renderer.list(i, l, o); case "list_item_start": for (i = "", this.token.task && (i += this.renderer.checkbox(this.token.checked)); this.next().type !== "list_item_end";) {
      i += this.token.type === "text" ? this.parseText() : this.tok()
    }

      return this.renderer.listitem(i); case "loose_item_start": for (i = ""; this.next().type !== "list_item_end";) {
      i += this.tok()
    }

      return this.renderer.listitem(i); case "html": return this.renderer.html(this.token.text); case "paragraph": return this.renderer.paragraph(this.inline.output(this.token.text)); case "text": return this.renderer.paragraph(this.parseText())
    }
  }; var c = {},
    g = /^$|^[a-z][a-z0-9+.-]*:|^[?#]/i; function d() {} function f(e) {
    for (var n, r = 1, t; r < arguments.length; r++) {
    ...
    if (e == null) {
      ...
      r || (r = t, t = null); let s, i,
        l = (t = f({}, m.defaults, t || {})).highlight,
        h = 0; try {
        s = n.lex(e, t)
      } catch (e) {
        return r(e)
      }i = s.length; let p = function(e) {
        if (e) {
        ...
        !(function(e) {
          e.type !== "code" ? --i || p() : l(e.text, e.lang, function(t, n) {
            return t ? p(t) : n == null || n === e.text ? --i || p() : (e.text = n, e.escaped = !0, void (--i || p()))
          })
        }(s[h]))
      }
    } else {
      try {
        ...
        if (e.message += "\nPlease report this to https://github.com/markedjs/marked.", (t || m.defaults).silent) {
  ...
  }, m.defaults = m.getDefaults(), m.Parser = o, m.parser = o.parse, m.Renderer = i, m.TextRenderer = l, m.Lexer = n, m.lexer = n.lex, m.InlineLexer = s, m.inlineLexer = s.output, m.parse = m, typeof module !== "undefined" && typeof exports === "object" ? module.exports = m : typeof define === "function" && define.amd ? define(function() {
    return m
  }) : e.marked = m
